...
float pid_p_gain_roll = 3;               //Ganho da parte P (Proporcional) do Roll
float pid_i_gain_roll = 0.05;              //Ganho da parte I (Integrador) do Roll
float pid_d_gain_roll = 15;                //Ganho da parte D (Derivativo) do Roll
...
void setup(){
  ...
  //Cálculo da tensão atual da bateria (importante) [Divisor de tensão R1 = 1500K e R2 = 1K]
  //O diodo apresenta queda de tensão equivalente a 65 (decimal)
  //A bateria cheia (12,6V) deve mostrar na porta analógica 5V (valor máximo da porta analógica do Arduino)
...
void loop(){
  ...
  if(!auto_level){                                                          /////// AUTO BALANCEAMENTO ////////////
  ...
  if(battery_voltage < 1000 && battery_voltage > 600)digitalWrite(12, HIGH);// Aviso caso o nivel da bateria esteja muito baixo (6 a 10 V)
  ...
  if (start == 2){                                                          //Motores estão armados
    ...
    if (battery_voltage < 1240 && battery_voltage > 800){                   //Compensa a bateria apenas se tiver entre 8 e 12.4 Volts
      esc_1 += esc_1 * ((1240 - battery_voltage)/(float)7000);              //Compoensação para o ESC 1 para a descarga da bateria
      esc_2 += esc_2 * ((1240 - battery_voltage)/(float)7000);              //Compoensação para o ESC 2 para a descarga da bateria
      esc_3 += esc_3 * ((1240 - battery_voltage)/(float)7000);              //Compoensação para o ESC 3 para a descarga da bateria
      esc_4 += esc_4 * ((1240 - battery_voltage)/(float)7000);              //Compoensação para o ESC 4 para a descarga da bateria
    } 
