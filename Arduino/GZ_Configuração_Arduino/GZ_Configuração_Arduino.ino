/////CALIBRAÇÃO INICIAL (EEPROM) DO DRONE ABELHINHA. 		YOUTUBE: GRAVIDADE ZERO////
//-------------------------------------------------------------------------------------
//									Termos de uso									  |
...
//PORÉM, É NECESSÁRIO INFORMAR AOS PROGRAMA "CALIBRAÇÃO_ESC" E "CONTROLADORA_DE_VOO"  |
...
//ADAPTADO PARA O PROJETO ABELINHA													  |
...
byte lowByte, highByte, type, Giroscópio_address, error, clockspeed_ok;
byte channel_1_assign, channel_2_assign, channel_3_assign, channel_4_assign;
byte roll_axis, pitch_axis, yaw_axis;
byte receiver_check_byte, Giroscópio_check_byte;
...
float Giroscópio_pitch, Giroscópio_roll, Giroscópio_yaw;
float Giroscópio_roll_cal, Giroscópio_pitch_cal, Giroscópio_yaw_cal;
...
void setup(){
  ...
  delay(250);               // Tempo extra pro giroscópio iniciar
}
...
void loop(){
  ...
  if(error == 0){
    ...
    if(type == 0){
      ...
      delay(1000);
      if(search_Giroscópio(0x68, 0x0F) == 0xD3){
    ...
    if(type == 0){
      ...
      delay(1000);
      if(search_Giroscópio(0x69, 0x0F) == 0xD3){
    ...
    if(type == 0){
      ...
      delay(1000);
      if(search_Giroscópio(0x6A, 0x0F) == 0xD7){
        Serial.println(F("L3GD20H encontrado no endereço 0x6A"));
        type = 3;
        Giroscópio_address = 0x6A;
      }
    }
    ...
    if(type == 0){
     Serial.println(F("Procurando pela IMU L3GD20H no endereço 0x6B/107"));
      delay(1000);
      if(search_Giroscópio(0x6B, 0x0F) == 0xD7){
        Serial.println(F("L3GD20H encontrado no endereço 0x6B"));
        type = 3;
        Giroscópio_address = 0x6B;
      }
    }
    
    if(type == 0){
      Serial.println(F("Nenhum giroscópio encontrado!!! (ERROR 3)"));
      ...
      Serial.println(F("==================================================="));
      start_Giroscópio(); //Configuração
    }
  }
  
  //Se o giroscópio foi encontrado podemos prosseguir para a calibração
  if(error == 0){
    delay(3000);
    Serial.println(F(""));
    Serial.println(F("==================================================="));
    Serial.println(F("				Calibração do Giroscópio"));
    Serial.println(F("==================================================="));
    Serial.println(F("Não mecha o drone!! A calibração começa em 5 segundos"));
    delay(5000);
    Serial.println(F("Calibrando o giroscópio, isso demora +/- 8 seconds"));
    ...
    for (cal_int = 0; cal_int < 2000 ; cal_int ++){              //Take 2000 readings for calibration.
      ...
      Giroscópio_signalen();                                           //Read the Giroscópio output.
      Giroscópio_roll_cal += Giroscópio_roll;                                //Ad roll value to Giroscópio_roll_cal.
      Giroscópio_pitch_cal += Giroscópio_pitch;                              //Ad pitch value to Giroscópio_pitch_cal.
      Giroscópio_yaw_cal += Giroscópio_yaw;                                  //Ad yaw value to Giroscópio_yaw_cal.
      delay(4);                                                  //Wait 3 milliseconds before the next loop.
    }
    //Now that we have 2000 measures, we need to devide by 2000 to get the average Giroscópio offset.
    Giroscópio_roll_cal /= 2000;                                       //Divide the roll total by 2000.
    Giroscópio_pitch_cal /= 2000;                                      //Divide the pitch total by 2000.
    Giroscópio_yaw_cal /= 2000;                                        //Divide the yaw total by 2000.
    
    //Show the calibration results
    Serial.println(F(""));
    Serial.print(F("Axis 1 offset="));
    Serial.println(Giroscópio_roll_cal);
    Serial.print(F("Axis 2 offset="));
    Serial.println(Giroscópio_pitch_cal);
    Serial.print(F("Axis 3 offset="));
    Serial.println(Giroscópio_yaw_cal);
    Serial.println(F(""));
    
    Serial.println(F("==================================================="));
    Serial.println(F("Giroscópio axes configuration"));
    Serial.println(F("==================================================="));
  ...
  if(error == 0){
    ...
    delay(1000);
    if(Giroscópio_check_byte == 0b00000111){
      Serial.println(F("Giroscópio axes ok"));
    }
    else{
      Serial.println(F("Giroscópio exes verification failed!!! (ERROR 7)"));
      error = 1;
    }
  }     
  ...
  if(error == 0){
    ...
    EEPROM.write(32, Giroscópio_address);
    ...
    if(Giroscópio_address != EEPROM.read(32))error = 1;
...
//Search for the Giroscópio and check the Who_am_I register
byte search_Giroscópio(int Giroscópio_address, int who_am_i){
  Wire.beginTransmission(Giroscópio_address);
  Wire.write(who_am_i);
  Wire.endTransmission();
  Wire.requestFrom(Giroscópio_address, 1);
  timer = millis() + 100;
  while(Wire.available() < 1 && timer > millis());
  lowByte = Wire.read();
  address = Giroscópio_address;
  return lowByte;
}

void start_Giroscópio(){
  //Setup the L3G4200D or L3GD20H
  if(type == 2 || type == 3){
    Wire.beginTransmission(address);                             //Start communication with the Giroscópio with the address encontrado during search
    Wire.write(0x20);                                            //We want to write to register 1 (20 hex)
    Wire.write(0x0F);                                            //Set the register bits as 00001111 (Turn on the Giroscópio and enable all axis)
    Wire.endTransmission();                                      //End the transmission with the Giroscópio

    Wire.beginTransmission(address);                             //Start communication with the Giroscópio (adress 1101001)
    Wire.write(0x20);                                            //Start reading @ register 28h and auto increment with every read
    Wire.endTransmission();                                      //End the transmission
    Wire.requestFrom(address, 1);                                //Request 6 bytes from the Giroscópio
  ...
  if(type == 1){
    ...
    Wire.beginTransmission(address);                             //Start communication with the Giroscópio
    Wire.write(0x6B);                                            //PWR_MGMT_1 register
    Wire.write(0x00);                                            //Set to zero to turn on the Giroscópio
    Wire.endTransmission();                                      //End the transmission
    
    Wire.beginTransmission(address);                             //Start communication with the Giroscópio
    Wire.write(0x6B);                                            //Start reading @ register 28h and auto increment with every read
    Wire.endTransmission();                                      //End the transmission
    Wire.requestFrom(address, 1);                                //Request 1 bytes from the Giroscópio
    while(Wire.available() < 1);                                 //Wait until the 1 byte is received
    Serial.print(F("Register 0x6B is set to:"));
    Serial.println(Wire.read(),BIN);
    
    Wire.beginTransmission(address);                             //Start communication with the Giroscópio
    Wire.write(0x1B);                                            //Giroscópio_CONFIG register
...
void Giroscópio_signalen(){
  if(type == 2 || type == 3){
    Wire.beginTransmission(address);                             //Start communication with the Giroscópio
    Wire.write(168);                                             //Start reading @ register 28h and auto increment with every read
    Wire.endTransmission();                                      //End the transmission
    Wire.requestFrom(address, 6);                                //Request 6 bytes from the Giroscópio
    while(Wire.available() < 6);                                 //Wait until the 6 bytes are received
    lowByte = Wire.read();                                       //First received byte is the low part of the angular data
    highByte = Wire.read();                                      //Second received byte is the high part of the angular data
    Giroscópio_roll = ((highByte<<8)|lowByte);                         //Multiply highByte by 256 (shift left by 8) and ad lowByte
    if(cal_int == 2000)Giroscópio_roll -= Giroscópio_roll_cal;               //Only compensate after the calibration
    lowByte = Wire.read();                                       //First received byte is the low part of the angular data
    highByte = Wire.read();                                      //Second received byte is the high part of the angular data
    Giroscópio_pitch = ((highByte<<8)|lowByte);                        //Multiply highByte by 256 (shift left by 8) and ad lowByte
    if(cal_int == 2000)Giroscópio_pitch -= Giroscópio_pitch_cal;             //Only compensate after the calibration
    lowByte = Wire.read();                                       //First received byte is the low part of the angular data
    highByte = Wire.read();                                      //Second received byte is the high part of the angular data
    Giroscópio_yaw = ((highByte<<8)|lowByte);                          //Multiply highByte by 256 (shift left by 8) and ad lowByte
    if(cal_int == 2000)Giroscópio_yaw -= Giroscópio_yaw_cal;                 //Only compensate after the calibration
  }
  if(type == 1){
    Wire.beginTransmission(address);                             //Start communication with the Giroscópio
    ...
    Wire.requestFrom(address,6);                                 //Request 6 bytes from the Giroscópio
    while(Wire.available() < 6);                                 //Wait until the 6 bytes are received
    Giroscópio_roll=Wire.read()<<8|Wire.read();                        //Read high and low part of the angular data
    if(cal_int == 2000)Giroscópio_roll -= Giroscópio_roll_cal;               //Only compensate after the calibration
    Giroscópio_pitch=Wire.read()<<8|Wire.read();                       //Read high and low part of the angular data
    if(cal_int == 2000)Giroscópio_pitch -= Giroscópio_pitch_cal;             //Only compensate after the calibration
    Giroscópio_yaw=Wire.read()<<8|Wire.read();                         //Read high and low part of the angular data
    if(cal_int == 2000)Giroscópio_yaw -= Giroscópio_yaw_cal;                 //Only compensate after the calibration
  }
}
...
void check_receiver_inputs(byte movement){
  ...
  if(trigger == 0){
	...
	Serial.prinln(F("Verifique a conexão do recepetor nos GPIOs corretor e a bindagem com o RC"))
  }
...
void wait_for_receiver(){
  ...
  if(zero == 0){
    error = 1;
    Serial.println(F("."));
    Serial.println(F("No valid receiver signals encontrado!!! (ERROR 1)"));
  }
...
//Check if the angular position of a Giroscópio axis is changing within 10 seconds
void check_Giroscópio_axes(byte movement){
  byte trigger_axis = 0;
  float Giroscópio_angle_roll, Giroscópio_angle_pitch, Giroscópio_angle_yaw;
  //Reset all axes
  Giroscópio_angle_roll = 0;
  Giroscópio_angle_pitch = 0;
  Giroscópio_angle_yaw = 0;
  Giroscópio_signalen();
  timer = millis() + 10000;    
  while(timer > millis() && Giroscópio_angle_roll > -30 && Giroscópio_angle_roll < 30 && Giroscópio_angle_pitch > -30 && Giroscópio_angle_pitch < 30 && Giroscópio_angle_yaw > -30 && Giroscópio_angle_yaw < 30){
    Giroscópio_signalen();
    if(type == 2 || type == 3){
      Giroscópio_angle_roll += Giroscópio_roll * 0.00007;              //0.00007 = 17.5 (md/s) / 250(Hz)
      Giroscópio_angle_pitch += Giroscópio_pitch * 0.00007;
      Giroscópio_angle_yaw += Giroscópio_yaw * 0.00007;
    }
    if(type == 1){
      Giroscópio_angle_roll += Giroscópio_roll * 0.0000611;          // 0.0000611 = 1 / 65.5 (LSB degr/s) / 250(Hz)
      Giroscópio_angle_pitch += Giroscópio_pitch * 0.0000611;
      Giroscópio_angle_yaw += Giroscópio_yaw * 0.0000611;
    }
    
    delayMicroseconds(3700); //Loop is running @ 250Hz. +/-300us is used for communication with the Giroscópio
  }
  //Assign the moved axis to the orresponding function (pitch, roll, yaw)
  if((Giroscópio_angle_roll < -30 || Giroscópio_angle_roll > 30) && Giroscópio_angle_pitch > -30 && Giroscópio_angle_pitch < 30 && Giroscópio_angle_yaw > -30 && Giroscópio_angle_yaw < 30){
    Giroscópio_check_byte |= 0b00000001;
    if(Giroscópio_angle_roll < 0)trigger_axis = 0b10000001;
    else trigger_axis = 0b00000001;
  }
  if((Giroscópio_angle_pitch < -30 || Giroscópio_angle_pitch > 30) && Giroscópio_angle_roll > -30 && Giroscópio_angle_roll < 30 && Giroscópio_angle_yaw > -30 && Giroscópio_angle_yaw < 30){
    Giroscópio_check_byte |= 0b00000010;
    if(Giroscópio_angle_pitch < 0)trigger_axis = 0b10000010;
    else trigger_axis = 0b00000010;
  }
  if((Giroscópio_angle_yaw < -30 || Giroscópio_angle_yaw > 30) && Giroscópio_angle_roll > -30 && Giroscópio_angle_roll < 30 && Giroscópio_angle_pitch > -30 && Giroscópio_angle_pitch < 30){
    Giroscópio_check_byte |= 0b00000100;
    if(Giroscópio_angle_yaw < 0)trigger_axis = 0b10000011;
